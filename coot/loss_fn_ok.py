...
class CrossCLR(nn.Module):
    ...
    def forward(self, video_features, text_features, input_vid1, input_txt1, epoch):
        """
        Inputs shape (batch, embed_dim)
        ...
        self._dequeue_and_enqueue(video_features, text_features, input_vid1, input_txt1)
        ...
        diag = np.eye(batch_size)
        mask_vid = th.from_numpy((diag)).cuda()
        mask_txt = th.from_numpy((diag)).cuda()
        ...
        if multi_pos:
            positive_mask = self._get_positive_mask( video_features.shape[0])
            sim_mask_vid = (input_vid1 @ input_vid1.t()) * positive_mask
            sim_mask_txt= (input_txt1 @ input_txt1.t()) * positive_mask
            _, topkidx_vid = th.topk(sim_mask_vid, num_p, dim=1)
            topk_onehot_vid = th.zeros_like(sim_mask_vid)
            topk_onehot_vid.scatter_(1, topkidx_vid, 1)
            mask_vid[topk_onehot_vid.bool()] = mp_score

            _, topkidx_txt = th.topk(sim_mask_txt, num_p, dim=1)
            topk_onehot_txt = th.zeros_like(sim_mask_txt)
        ...
        # loss_i = th.empty(batch_size).cuda()
        # loss_t = th.empty(batch_size).cuda()
        # for i_b in range(batch_size):
        #     loss_i[i_b] = self.compute_loss(vid_logits_prune[i_b,:], mask_v[i_b,:])
        #     loss_t[i_b] = self.compute_loss(txt_logits_prune[i_b,:], mask_t[i_b,:])
        ...
        # print(w_i, th.exp(w_i/ self.temp_w))
        # print(w_t, th.exp(w_t/ self.temp_w))
        # print("=="*30)
