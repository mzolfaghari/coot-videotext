...
class CrossCLR(nn.Module):
    ...
    def forward(self, video_features, text_features, input_vid, input_txt):
        ...
        input_vid2 = th.mean(input_vid, dim=1)
        ...
        self._dequeue_and_enqueue(video_features, text_features, input_vid2, input_txt2)
        ...
        logits_per_video /= self.temperature #* self.logit_scale.exp()
        ...
        # positive_mask =  self._get_positive_mask_bank(self.K, video_features.shape[0], self.batch_start, self.batch_end) #self._get_positive_mask(self.K)[self.queue_ptr[0]:qptr_end,:]
        ...
        sim_scores_vid = (input_vid2 @ input_vid2.t()) * positive_mask_batch
        sim_scores_txt= (input_txt2 @ input_txt2.t()) * positive_mask_batch
        ...
        sorted_vid = self._normalize(sorted_vid)
        sorted_txt = self._normalize(sorted_txt)
        # import pdb; pdb.set_trace()
        ...
        img_logits_prune = th.cat([logits_per_video, w_negative * negatives_vid], dim=1)
        txt_logits_prune = th.cat([logits_per_text, w_negative * negatives_txt], dim=1)

        
        labels_prune = th.arange(logits_per_video.shape[0]).cuda()
