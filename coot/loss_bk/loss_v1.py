...
class CrossCLR(nn.Module):
    ...
    def __init__(self, temperature=0.05, logger = None):
        ...
        self.temperature = 0.05 #temperature
        self.logger = logger
        self.K = 3000
        dim = 384 #network embedding size
        dim_org_vid = 512
    ...
    def _dequeue_and_enqueue(self, keys_vid, keys_txt, keys_vid_org, keys_txt_org):
        ...
        self.batch_start = ptr
        self.batch_end = ptr_end
    ...
    def forward(self, image_features, text_features, input_vid, input_txt):
        ...
        positive_mask =  self._get_positive_mask_bank(self.K, image_features.shape[0], self.batch_start, self.batch_end) #self._get_positive_mask(self.K)[self.queue_ptr[0]:qptr_end,:]
        ...
        if weighted_loss == 0:
            ...
            if cls_method == 6:
                ...
                if intra_modality:
                    ...
                    # x1 = image_features @ text_features.t() / self.temperature
                    # x2 = text_features @ image_features.t() / self.temperature
                    # vid_logits_prune = th.cat([x1, w_negative * negatives_vid], dim=1)
                    # txt_logits_prune = th.cat([x2, w_negative * negatives_txt], dim=1)

                    # eq_fl = x1.allclose(logits_per_image[:, :batch_size])
                    # if eq_fl:
                        # print("PTR: ", self.queue_ptr[0])
                    # else:
                    #     print("PTR: ", self.queue_ptr[0])
                    # import pdb; pdb.set_trace()
                ...
                # mask_pos_v = th.zeros_like(logits_per_image)
                # mask_pos_t = th.zeros_like(logits_per_image)

                # # Find top similar samples among negative ones
                # _, topkidx_v = th.topk(sim_scores_vid, self.topk, dim=1)
                # _, topkidx_t = th.topk(sim_scores_txt, self.topk, dim=1)
                # topk_onehot_v = th.zeros_like(mask_pos_v)
                # topk_onehot_v.scatter_(1, topkidx_v, 1)
                # mask_pos_v[topk_onehot_v.bool()] = True
                # mask_pos_v[:batch_size,:batch_size] = th.eye(batch_size, batch_size).bool()

                # topk_onehot_t = th.zeros_like(mask_pos_t)
                # topk_onehot_t.scatter_(1, topkidx_t, 1)
                # # similarity_matrix[self.mask_samples_from_same_repr].view(2 * self.batch_size, -1)
                # mask_pos_t[topk_onehot_t.bool()] = True
                # mask_pos_t[:batch_size,:batch_size] = th.eye(batch_size).bool()

                # mask_neg_v = th.zeros_like(negatives_vid)
                # mask_neg_t = th.zeros_like(negatives_txt)


                # mask_v = th.cat([mask_pos_v, mask_neg_v], dim=1)
                # mask_t = th.cat([mask_pos_t, mask_neg_t], dim=1)
                # x1 = image_features @ text_feat
                # import pdb;pdb.set_trace()
                ...
                # diag = np.eye(batch_size, dtype=np.float64)
                # mask_v = th.from_numpy((diag)).cuda()
                # mask_t = th.from_numpy((diag)).cuda()

                # topk_onehot_v = th.zeros_like(x1).detach()
                # topk_onehot_t = th.zeros_like(x1).detach()

                # mask_sim = input_vid2.matmul(input_vid2.t())
                # _, topkidx_v = th.topk(mask_sim, self.topk, dim=1)
                # topk_onehot_v.scatter_(1, topkidx_v, 1)
                # mask_v[topk_onehot_v.bool()] = True

                # mask_sim = input_txt2.matmul(input_txt2.t())
                # _, topkidx_t = th.topk(mask_sim, self.topk, dim=1)
                # topk_onehot_t.scatter_(1, topkidx_t, 1)
                # mask_t[topk_onehot_t.bool()] = True

                # # import pdb; pdb.set_trace()
                # # mask = th.eye(batch_size, dtype=th.float64)[:, :(self.batch_end- self.batch_start)].cuda().bool()
                # mask_neg_v = th.zeros_like(negatives_vid, dtype=th.float64).detach().bool()
                # mask_neg_t = th.zeros_like(negatives_txt, dtype=th.float64).detach().bool()
                # # mask_spos = th.ones(batch_size, 2, dtype=th.float32)
                # mask_v = th.cat([mask_v, mask_neg_v], dim=1)
                # mask_t = th.cat([mask_t, mask_neg_t], dim=1)
                # # print(vid_logits_prune.shape, mask_v.shape)
                # # import pdb;pdb.set_trace()
                # loss_i = self.compute_loss(vid_logits_prune, mask_v)
                # loss_t = self.compute_loss(txt_logits_prune, mask_t)
