...
class CrossCLR(nn.Module):
    ...
    def _get_positive_mask_bank(self, k, batch_size, ptr_start, ptr_end):
        ...
        # print(ptr_start,ptr_end, ptr_end-ptr_start, batch_size)
        # import pdb; pdb.set_trace()
    ...
    def _dequeue_and_enqueue(self, keys_vid, keys_txt, keys_vid_org, keys_txt_org):
        ...
        # print(ptr, ptr_end, ptr_end_k)
        # import pdb; pdb.set_trace()
        ptr_x = ptr
        if ptr_end_k != batch_size:
            ptr_x = ptr + ptr_end_k
        # print(ptr,ptr_end, ptr_end_k, ptr_x)
    ...
    def forward(self, image_features, text_features, input_img, input_txt):
        ...
        input_img2 = th.mean(input_img, dim=1)
        input_txt2 = th.mean(input_txt, dim=1)
        ...
        if self.queue_is_full:
            logits_clstr_img = image_features @ self.queue_vid.detach().cuda()
            logits_clstr_txt = text_features @ self.queue_txt.detach().cuda()

            # logits_per_image = image_features @ self.queue_txt.detach().cuda()
            # logits_per_text = text_features @ self.queue_vid.detach().cuda()
            ...
            positive_mask =  self._get_positive_mask_bank(self.K, image_features.shape[0], self.batch_start, self.batch_end)
            ind_start = self.batch_start
            # labels = th.arange(image_features.shape[0]).cuda() + self.batch_start
            # import pdb; pdb.set_trace()
            # labels = labels_tmp[:(self.batch_end-self.batch_start)]

            # mask_sim = kf.matmul(self.queue_second.clone().detach())
            # mask_sim[mask_source] = - np.inf # mask out self (and sibling videos)
            # _, topkidx_vid = th.topk(logits_per_image, 1, dim=1)
            # _, topkidx_txt = th.topk(logits_per_text, 1, dim=1)
            # import pdb; pdb.set_trace()
            # topk_onehot = torch.zeros_like(logits_per_image)
            # topk_onehot.scatter_(1, topkidx, 1)
            # mask[topk_onehot.bool()] = True

            # print(self.batch_start, self.batch_end, image_features.shape, len(labels))
        else:
            # print(self.batch_start, self.batch_end)
        ...
        # avg_sim_img_q = th.mean(sim_scores_img,dim=0)
        # avg_sim_txt_q = th.mean(sim_scores_txt,dim=0)
        ...
        indices_img_thrsh = indices_img[sorted_img>thrshold] + ind_start
        indices_txt_thrsh = indices_txt[sorted_txt>thrshold] + ind_start 
        # import pdb; pdb.set_trace()
        ...
        # logits_clstr_img[:, indices_img_thrsh] = 0 #- np.inf
