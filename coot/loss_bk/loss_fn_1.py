...
class CrossCLR(nn.Module):
    ...
    @th.no_grad()
    def _dequeue_and_enqueue_head(self, keys_vid, keys_txt, keys_vid_org, keys_txt_org):

        batch_size = keys_vid.shape[0]

        ptr = int(self.queue_ptr)
        self.queue_vid[:, batch_size:] = self.queue_vid[:, :-batch_size].detach().clone()#.detach()
        self.queue_txt[:, batch_size:] = self.queue_txt[:, :-batch_size].detach().clone()#.detach()
        self.queue_vid_org[:, batch_size:] = self.queue_vid_org[:, :-batch_size].detach().clone()#.detach()
        self.queue_txt_org[:, batch_size:] = self.queue_txt_org[:, :-batch_size].detach().clone()#.detach()
        self.queue_vid[:, :batch_size] = keys_vid.T
        self.queue_txt[:, :batch_size] = keys_txt.T
        self.queue_vid_org[:, :batch_size] = keys_vid_org.T
        self.queue_txt_org[:, :batch_size] = keys_txt_org.T
        ptr = (ptr + batch_size) % self.K  # move pointer
    ...
    def forward(self, video_features, text_features, input_vid, input_txt):
        ...
        #======
        # thrsh_vid = 0.8 #0.012 #threshold for vid similarity - find influential samples
        ...
        self._dequeue_and_enqueue_head(video_features, text_features, input_vid2, input_txt2)

        logits_per_vid = video_features @ text_features.t()
        logits_per_text = text_features @ video_features.t()
        # logits_per_vid = video_features @ self.queue_txt.clone().detach().cuda()
        # logits_per_text = text_features @ self.queue_vid.clone().detach().cuda()

        # l_neg = th.einsum('nc,ck->nk', [text_features, self.queue_vid.clone().detach().cuda()])

        # log1 = video_features @ text_features.t()
        # import pdb; pdb.set_trace()

        logits_clstr_vid = video_features @ video_features.t()
        logits_clstr_txt = text_features @ text_features.t()

        # logits_clstr_vid = video_features @ self.queue_vid.cuda()
        # # logits_clstr_vid = video_features @ video_features.t()
        # logits_clstr_txt= text_features @ self.queue_txt.cuda()
        # logits_clstr_txt = text_features @ text_features.t()

        logits_per_vid /= self.temperature #* self.logit_scale.exp()
        ...
        qptr_end = self.queue_ptr[0]+video_features.shape[0]
        if (self.queue_ptr[0]+video_features.shape[0]) > self.K:
            qptr_end = self.K
            inp_feat_k = video_features.shape[0] - (self.queue_ptr[0]+video_features.shape[0] - self.K)
            positive_mask =  self._get_positive_mask_bank(self.K, video_features.shape[0], self.queue_ptr[0])
            # import pdb; pdb.set_trace()
            # sim_scores_vid = (input_vid2[:inp_feat_k,:] @ self.queue_vid_org.cuda()) * positive_mask
            # sim_scores_txt= (input_txt2[:inp_feat_k,:] @ self.queue_txt_org.cuda()) * positive_mask
            # logits_per_vid = logits_per_vid[:inp_feat_k,:]
            # logits_per_text = logits_per_text[:inp_feat_k,:]
            # logits_clstr_vid = logits_clstr_vid[:inp_feat_k,:]
            # logits_clstr_txt = logits_clstr_txt[:inp_feat_k,:]
        else:
            positive_mask =  self._get_positive_mask_bank(self.K, video_features.shape[0], self.queue_ptr[0]) #self._get_positive_mask(self.K)[self.queue_ptr[0]:qptr_end,:]
        ...
        if weighted_loss == 0:
            # ---- CLIP ---
            # logits_per_vid = logits_per_vid / 0.05 #self.logit_scale.exp()
            # logits_per_text = logits_per_text / 0.05 #self.logit_scale.exp()
            # logits_clstr_vid = self.logit_scale.exp()
            # logits_clstr_txt = self.logit_scale.exp()
            positive_mask =  self._get_positive_mask(video_features.shape[0]).type(th.bool)
            logits_clstr_vid = logits_clstr_vid * positive_mask
            logits_clstr_txt = logits_clstr_txt * positive_mask
            labels = th.arange(video_features.shape[0]).cuda()
            

            if intra_modality:
                if no_softmax:  
                    vid_logits_prune = th.cat([logits_per_vid, w_negative * logits_clstr_vid], dim=1)
                    txt_logits_prune = th.cat([logits_per_text, w_negative * logits_clstr_txt], dim=1)
                else: 
                    vid_logits_prune = th.cat([F.softmax(logits_per_vid, dim=1), w_negative * F.softmax(logits_clstr_vid, dim=1)], dim=1)
                    txt_logits_prune = th.cat([F.softmax(logits_per_text, dim=1), w_negative * F.softmax(logits_clstr_txt, dim=1)], dim=1)
            else:
                if no_softmax:  
                    vid_logits_prune = logits_per_vid
                    txt_logits_prune = logits_per_text
                else: 
                    vid_logits_prune = F.softmax(logits_per_vid, dim=1)
            ...
            # sim_scores_vid = (input_vid2 @ input_vid2.t()) * positive_mask
            ...
            ##----- Dist ----
            # sim_scores_vid = (logits_per_vid) * positive_mask
            ...
            # if (self.queue_ptr[0]+video_features.shape[0]) > self.K:
            #     inp_feat_k = video_features.shape[0] - (self.queue_ptr[0]+video_features.shape[0] - self.K)
            #     indices_vid_thrsh = indices_vid[sorted_vid<thrshold] + self.queue_ptr[0] - inp_feat_k
            #     indices_txt_thrsh = indices_txt[sorted_txt<thrshold] + self.queue_ptr[0] - inp_feat_k
            # else:
            indices_vid_thrsh = indices_vid[sorted_vid<thrshold]# + self.queue_ptr[0]
            indices_txt_thrsh = indices_txt[sorted_txt<thrshold]# + self.queue_ptr[0]
            # indices_vid_thrsh = indices_vid_thrsh[indices_vid_thrsh<self.K]
            # indices_txt_thrsh = indices_txt_thrsh[indices_txt_thrsh<self.K]
            # import pdb; pdb.set_trace()

            # indices_vid_thrsh = indices_txt_thrsh

            # # import pdb; pdb.set_trace()
            # print("vid:", sorted_vid[1],sorted_vid[-1])  #0.012
            # print("Text:", sorted_txt[1],sorted_txt[-1])   #90
            # print("==="*20)    
            labels = th.arange(video_features.shape[0]).cuda()
            # print(logits_per_vid.shape)

            # print(labels)
            # vid_logits_prune = logits_per_vid[:, indices_txt[n_prune:]][indices_txt[n_prune:],:]
            ...
            if cls_method == 6:
                ...
                positive_mask_batch =  self._get_positive_mask(logits_clstr_vid.shape[0]).type(th.bool)
                # positive_mask =  self._get_positive_mask(self.K)[self.queue_ptr[0]:self.queue_ptr[0]+logits_clstr_vid.shape[0],:].type(th.bool)
                logits_clstr_vid = logits_clstr_vid * positive_mask_batch
                logits_clstr_txt = logits_clstr_txt * positive_mask_batch
                ...
                # vid_logits_prune = th.cat([F.softmax(logits_per_vid, dim=1), F.softmax(logits_clstr_vid, dim=1)], dim=1)
                # txt_logits_prune = th.cat([F.softmax(logits_per_text, dim=1), F.softmax(logits_clstr_txt, dim=1)], dim=1) 
                if intra_modality:
                    if no_softmax:  
                        # vid_logits_prune = th.cat([logits_per_vid, w_negative * negatives_vid, w_negative * negatives_txt], dim=1)
                        # txt_logits_prune = th.cat([logits_per_text, w_negative * negatives_txt, w_negative * negatives_vid], dim=1)
                        vid_logits_prune = th.cat([logits_per_vid, w_negative * negatives_vid], dim=1)
                        txt_logits_prune = th.cat([logits_per_text, w_negative * negatives_txt], dim=1)
                    else: 
                        # vid_logits_prune = th.cat([F.softmax(logits_per_vid, dim=1), w_negative * F.softmax(negatives_vid, dim=1), w_negative * F.softmax(negatives_txt, dim=1)], dim=1)
                        # txt_logits_prune = th.cat([F.softmax(logits_per_text, dim=1), w_negative * F.softmax(negatives_txt, dim=1), w_negative * F.softmax(negatives_vid, dim=1)], dim=1)
                        vid_logits_prune = th.cat([F.softmax(logits_per_vid, dim=1), w_negative * F.softmax(negatives_vid, dim=1)], dim=1)
                ...
                labels_prune = th.arange(video_features.shape[0]).cuda()
                # print(labels_prune)
                ...
                if intra_modality:
                    if no_softmax:  
                        # vid_logits_prune = th.cat([logits_per_vid, w_negative * negatives_vid, w_negative * negatives_txt], dim=1)
                        # txt_logits_prune = th.cat([logits_per_text, w_negative * negatives_txt, w_negative * negatives_vid], dim=1)
                        vid_logits_prune1 = logits_per_vid
                        vid_logits_prune2 = th.cat([th.diag(logits_per_vid).view(logits_per_vid.shape[0], 1), w_negative * negatives_vid], dim=1)
                        txt_logits_prune1 = logits_per_text
                        txt_logits_prune2 = th.cat([th.diag(logits_per_text).view(logits_per_vid.shape[0], 1), w_negative * negatives_txt], dim=1)
                    else: 
                        # vid_logits_prune = th.cat([F.softmax(logits_per_vid, dim=1), w_negative * F.softmax(negatives_vid, dim=1), w_negative * F.softmax(negatives_txt, dim=1)], dim=1)
                        # txt_logits_prune = th.cat([F.softmax(logits_per_text, dim=1), w_negative * F.softmax(negatives_txt, dim=1), w_negative * F.softmax(negatives_vid, dim=1)], dim=1)
                        vid_logits_prune = th.cat([F.softmax(logits_per_vid, dim=1), w_negative * F.softmax(negatives_vid, dim=1)], dim=1)
                    ...
                    labels_prune1 = th.arange(video_features.shape[0]).cuda()
                    labels_prune2 = th.zeros(video_features.shape[0]).cuda(non_blocking=True).long()
