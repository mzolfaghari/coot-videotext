...
class CrossCLR(nn.Module):
    ...
    def forward(self, image_features, text_features, input_vid, input_txt):
        ...
        w_negative = 0.7 #0.8 weight of negative scores.
        ...
        if weighted_loss == 0:
            ...
            if cls_method == 6:
                ...
                # positive_mask =  self._get_positive_mask(batch_size).type(th.bool)
                ...
                if intra_modality:
                    ...
                    # topk_onehot_t = th.zeros_like(logits_per_image)
                    # mask_sim = input_txt2.matmul(input_txt2.t()) * positive_mask
                    # _, topkidx_t = th.topk(mask_sim, self.topk, dim=1)
                    # topk_onehot_t.scatter_(1, topkidx_t, 1)

                    # topk_onehot_v = th.zeros_like(logits_per_image)
                    # mask_sim = input_vid2.matmul(input_vid2.t()) * positive_mask
                    # _, topkidx_v = th.topk(mask_sim, self.topk, dim=1)
                    # topk_onehot_v.scatter_(1, topkidx_v, 1)

                    # logits_vid1 = logits_per_image[topk_onehot_v.bool()].view(batch_size, 1)
                    # logits_txt1 = logits_per_text[topk_onehot_t.bool()].view(batch_size, 1)

                    # # import pdb; pdb.set_trace()

                    # vid_logits_prune_semi = th.cat([logits_vid1, w_negative * logits_clstr_vid], dim=1)
                    # txt_logits_prune_semi = th.cat([logits_txt1, w_negative * logits_clstr_txt], dim=1)
                ...
                # labels_prune_semi = th.zeros(batch_size).cuda(non_blocking=True).long()
                # loss_i_semi = self.criterion(vid_logits_prune_semi, labels_prune_semi)
                # loss_t_semi = self.criterion(txt_logits_prune_semi, labels_prune_semi) 
            ...
            # loss_i_semi = loss_i_semi * th.exp(w_i/ w_temp)
            # loss_t_semi = loss_t_semi * th.exp(w_t/ w_temp)

            loss_i = sum(loss_i) / (sum(th.exp(w_i/ w_temp)))
            loss_t = sum(loss_t) / (sum(th.exp(w_t/ w_temp)))

            # loss_i_semi  = sum(loss_i_semi ) / (sum(th.exp(w_i/ w_temp)))
            # loss_t_semi  = sum(loss_t_semi ) / (sum(th.exp(w_t/ w_temp)))
        ...
        loss = ((loss_i + loss_t)  )  / 2 #+ 0.3 * ((loss_i_semi.mean() + loss_t_semi.mean() )  )  / 2 #0.5 * ((loss_i_semi + loss_t_semi)  )  / 2
