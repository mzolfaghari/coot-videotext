...
class CrossCLR(nn.Module):
    ...
    def __init__(self, temperature=0.03, temperature_weights=0.0035, negative_weight=0.8, 
        score_thrshold=0.7, out_emb_dim=384, org_vid_dim=512, org_txt_dim=1536, K=3000, logger = None):
        super().__init__()
        ...
        self.logger.info("==="*30)
        self.logger.info("Queue size:{}, Temp:{}, TempW:{}, NegW:{}, Sthrsh:{}".format(self.K,self.temperature,self.temp_w,self.negative_w,self.score_thrshold))
        self.logger.info("==="*30)
        # create the queue
    ...
    def compute_loss(self, logits, mask):
        ...
        loss = - th.log( (F.softmax(logits, dim=0) * mask).sum() )
    ...
    def _get_positive_mask_bank2(self, k, batch_size):
        diag = np.eye(batch_size)
        mask = th.from_numpy((diag))
        # mask = (1 - mask)
        ...
        mask_bank[:, ptr_x:ptr_x+batch_size] -= mask


        return mask_bank.cuda(non_blocking=True)

    @th.no_grad()
    def _dequeue_and_enqueue2(self, keys_vid, keys_txt, keys_vid_org, keys_txt_org):
        ...
        self.queue_vid_org[:, ptr_x:ptr_x+batch_size] = keys_vid_org.T
        self.queue_txt_org[:, ptr_x:ptr_x+batch_size] = keys_txt_org.T
    ...
    def forward(self, video_features, text_features, input_vid, input_txt, epoch):
        """
        Inputs shape (batch, embed_dim)
        ...
        self._dequeue_and_enqueue2(video_features, text_features, input_vid1, input_txt1)

        # logits_per_image = video_features @ self.queue_txt.cuda() #text_features.t()
        # logits_per_text = text_features @ self.queue_vid.cuda() #video_features.t()
        ...
        # logits_per_image2 /= self.temperature 
        # logits_per_text2 /= self.temperature
        ...
        # positive_mask =  self._get_positive_mask_bank2(self.K, video_features.shape[0])
        # positive_mask = self._get_positive_mask( video_features.shape[0])
        sim_scores_vid = (input_vid1 @ self.queue_vid_org.cuda()) #* positive_mask
        sim_scores_txt= (input_txt1 @ self.queue_txt_org.cuda()) #* positive_mask
        ...
        sorted_vid2, indices_vid2 = th.sort(avg_sim_vid)
        sorted_txt2, indices_txt2 = th.sort(avg_sim_txt)
        ...
        # logits_per_vid = logits_per_image[:, indices_vid_thrsh]
        # logits_per_text = logits_per_text[:, indices_txt_thrsh]



        batch_size = input_vid1.shape[0]
        labels_prune = th.arange(batch_size).cuda()
        
        diag = np.eye(batch_size)
        mask = th.from_numpy((diag)).cuda()
        mask_prune_pos_vid = th.ones_like(logits_per_image)
        mask_prune_pos_txt = th.ones_like(logits_per_text)
        # print(mask_prune_pos[0,:100])
        # print("-1-"*20)
        ...
        # print(logits_per_image[0,:200])
        # print("-4-"*20)
        # print(vid_logits_prune[0,:200])
        # print("-5-"*20)

        vid_logits = th.cat([vid_logits_prune, self.negative_w * negatives_vid], dim=1)
        txt_logits = th.cat([txt_logits_prune, self.negative_w * negatives_txt], dim=1)


        # _, topkidx = th.topk(sim_scores_vid, 2, dim=1)
        # topk_onehot = th.zeros_like(sim_scores_vid)
        # mask = th.zeros_like(sim_scores_vid)
        # topk_onehot.scatter_(1, topkidx, 1)
        # mask[topk_onehot.bool()] = 1
        # import pdb;pdb.set_trace()


        mask_vid = th.zeros_like(logits_per_image)
        mask_txt = th.zeros_like(logits_per_text)
        # print(mask_prune_pos[0,:100])
        # print("-1-"*20)
        mask_vid[:, indices_vid_thrsh2] = 1
        mask_txt[:, indices_txt_thrsh2] = 1
        for i in range(batch_size): 
            if mask_vid[i,i]==1: mask_vid[i,i]=0
            if mask_txt[i,i]==1: mask_txt[i,i]=0

        mask_neg_v = th.zeros_like(negatives_vid)
        mask_neg_t = th.zeros_like(negatives_txt)
        # mask_neg_q_v = th.zeros_like(logits_per_image2)
        # mask_neg_q_t = th.zeros_like(negatives_txt)
        mask_v = th.cat([mask_vid, mask_neg_v], dim=1)
        mask_t = th.cat([mask_txt, mask_neg_t], dim=1)
        # import pdb;pdb.set_trace()

        loss_i2 = th.empty(batch_size).cuda()
        loss_t2 = th.empty(batch_size).cuda()
        for i_b in range(batch_size):
            vid_logits[i_b, i_b] = 0 
            txt_logits[i_b, i_b] = 0
            loss_i2[i_b] = self.compute_loss(vid_logits[i_b,:], mask_v[i_b,:])
            loss_t2[i_b] = self.compute_loss(txt_logits[i_b,:], mask_t[i_b,:])
        ...
        loss_i = self.criterion(vid_logits, labels_prune)
        loss_t = self.criterion(txt_logits, labels_prune)
        ...
        loss = ((loss_i + loss_t)  )  / 2  + 0.5 * (loss_i2.mean() + loss_t2.mean())/2
