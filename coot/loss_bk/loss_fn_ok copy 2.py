...
class CrossCLR(nn.Module):
    ...
    def __init__(self, temperature=0.03, temperature_weights=0.0035, negative_weight=0.8, 
        ...
        self.temperature = temperature #temperature #0.04
        self.logger = logger
    ...
    def forward(self, image_features, text_features, input_img, input_txt, epoch):
        """
        Inputs shape (batch, embed_dim)
        ...
        # print(image_features.shape, input_img1.shape)
        # if epoch > 25:
        #     input_img2 = image_features
        #     input_txt2 = text_features
        # else:
        ...
        sim_scores_img_g = (self.queue_vid_org.t().cuda() @ self.queue_vid_org.cuda()) #* positive_mask
        sim_scores_txt_g= (self.queue_txt_org.t().cuda() @ self.queue_txt_org.cuda()) #* positive_mask
        ...
        if weighted_loss == 1:
            ...
            avg_sim_img_g = th.mean(sim_scores_img_g,dim=0)
            avg_sim_txt_g = th.mean(sim_scores_txt_g,dim=0)

            sorted_img, indices_img = th.sort(avg_sim_img_g)
            ...
            indices_img_thrsh = indices_img[sorted_img<self.score_thrshold]
            ...
            if cls_method == 6:
                ...
                img_logits_prune = th.cat([logits_per_image, self.negative_w * negatives_img], dim=1)
